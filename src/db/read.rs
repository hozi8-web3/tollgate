...
pub fn get_requests(db: &DbPool, limit: i64, offset_val: i64) -> Result<Vec<RequestRow>> {
    ...
    let rows = stmt.query_map([limit, offset_val], |r| {
        Ok(RequestRow {
            ...
            provider: r.get(2)?,
            model: r.get(3)?,
            original_model: r.get(4)?,
            was_substituted: r.get::<_, i32>(5)? != 0,
            input_tokens: r.get(6)?,
            output_tokens: r.get(7)?,
            cache_read_tokens: r.get(8)?,
            cache_write_tokens: r.get(9)?,
            input_cost_usd: r.get(10)?,
            output_cost_usd: r.get(11)?,
            cache_read_cost_usd: r.get(12)?,
            cache_write_cost_usd: r.get(13)?,
            total_cost_usd: r.get(14)?,
            latency_ms: r.get(15)?,
            stop_reason: r.get(16)?,
            task_type: r.get(17)?,
            tags: r.get(18)?,
            anomaly: r.get::<_, i32>(19)? != 0,
            anomaly_reason: r.get(20)?,
        })
    })?;
    rows.collect::<std::result::Result<Vec<_>, _>>()
        .map_err(Into::into)
...
pub fn get_anomaly_stats(db: &DbPool, period_days: i64) -> Result<AnomalyStats> {
    ...
    let (c, h) = stmt.query_row([&offset], |r| {
        Ok((r.get::<_, i64>(0)?, r.get::<_, f64>(1)?))
    })?;
    Ok(AnomalyStats {
        anomalies_count: c,
        highest_single_request_usd: h,
    })
}
...
pub fn get_cache_stats(db: &DbPool, period_days: i64) -> Result<CacheStats> {
    ...
    let mut stmt = conn.prepare(
        "SELECT COUNT(*), COALESCE(SUM(CASE WHEN cache_read_tokens>0 THEN 1 ELSE 0 END),0), COALESCE(SUM(cache_read_cost_usd),0)
         FROM requests WHERE timestamp >= datetime('now', ?1)")?;
    let (total, hits, savings) = stmt.query_row([&offset], |r| {
        Ok((
            r.get::<_, i64>(0)?,
            r.get::<_, i64>(1)?,
            r.get::<_, f64>(2)?,
